...
dataload=DataLoader(my_datasets_train,batch_size=8,shuffle=True)
dataload_train=DataLoader(my_datasets_train,batch_size=209)
my_datasets_test=my_data(test_set_x_orig,test_set_y)
dataload_test=DataLoader(my_datasets_test,batch_size=50)
...
for i,(x,y) in enumerate(dataload_train):
        ...
        a=acc(y_label,y)
        print(a)
for i,(x,y) in enumerate(dataload_test):
        x=x.to(device)
        y_label=my_model.forwad(x)
        y_label=pred(y_label)
        a=acc(y_label,y)
